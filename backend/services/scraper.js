...
async function scrapeUrl(url) {
    ...
    try {
        // Headers para fingir ser un bot social (Discord/Facebook) y evitar bloqueos
        const { data } = await axios.get(url, {
            ...
            timeout: 15000 // 15s por si la red va lenta
        });
        ...
        $('script[type="application/ld+json"]').each((i, el) => {
            try {
                ...
                // Error silencioso parseando un bloque JSON (no importa, seguimos)
            }
        });
        ...
        return {
            image: null,
            text: "Error accediendo a URL. La IA deberá trabajar sin datos previos.",
            error: true
        };
    }
}
